...
def gaussiankernel ():
    ...
    print('kernel of gaussian',kernel)
    minn = np.min(kernel)
    for a in range(m):
        for b in range(n):
            kernel[a][b] = round(kernel[a][b]/summ/minn)
...
outimg = cv2.resize(img, (output_row_num,output_col_num), interpolation = cv2.INTER_AREA)
...
doubly_blocked = np.zeros( doubly_blocked_shape, dtype='uint64' )
...
for i in range(output.shape[0]):
    for j in range(output.shape[1]):
        outimg.itemset((i,j),round(output[i][j])/summ)
